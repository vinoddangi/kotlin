...
private fun defineClasses(
    ...
    CompilingEvaluatorUtils.defineClass(MagicFunctionFileObject.name, MagicFunctionFileObject.bytes, context, process, classLoader)
...
private object MagicFunctionFileObject {
    val bytes: ByteArray by Delegates.lazy {
        ...
        if (inputStream != null) {
            try {
                return@lazy CompilingEvaluatorUtils.changeSuperToMagicAccessor(inputStream.readBytes())
            }
            finally {
                inputStream.close()
            }
